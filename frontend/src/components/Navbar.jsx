...
function Navbar() {
  ...
  const toggleMode = () => {
    ...
    document.body.classList.toggle("dark-mode", !darkMode); // Optional: Apply global class to body
  };

  return (
    <>
    <div className={`w-full h-full flex justify-center items-center md:px-16 p-5 md:p-6 ${darkMode ? 'bg-[#09090b]' : 'bg-gray-200'}`}>
      <div className="w-full h-full">
        <p className={`text-xl font-bold ${darkMode ? 'text-yellow-400' : 'text-black'}`}>StudentPortal</p>
      </div>
      <div className="gap-7 md:flex p-1">
        <a href="/" className={`hover:text-blue-300 transition text-lg ${darkMode ? 'text-white' : 'text-black'} font-semibold duration-300`}>
          Home
        </a>
        <a href="/login" className={`hover:text-blue-300 transition text-lg ${darkMode ? 'text-white' : 'text-black'} font-semibold duration-300`}>
          Login
        </a>
        <a href="/register" className={`hover:text-blue-300 transition text-lg ${darkMode ? 'text-white' : 'text-black'} font-semibold duration-300`}>
          Register
        </a>
      </div>
      {/* Toggle Switch */}
